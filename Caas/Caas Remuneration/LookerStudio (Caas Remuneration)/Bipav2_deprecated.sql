...
WITH 
  ConfirmationDates AS (
    SELECT
      ...
      CONCAT(endToEndId, CAST(logCreated AS STRING)) AS InternalId,
    FROM
      PurchasesLog
    WHERE 
      rowNumber = 1
  ...
  ReversedLogsRaw AS (
    SELECT
      endToEndId,
      logAmount,
      LAST_VALUE(logCreated) OVER(PARTITION BY endToEndId ORDER BY logCreated ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS referenceDate,
      CONCAT(endToEndId,CAST(LAST_VALUE(logCreated) OVER(PARTITION BY endToEndId ORDER BY logCreated ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS STRING)) AS internalId,
      type,
    FROM
      PurchasesLog
  ),
  ReversedLogs AS (
    SELECT
      ...
      logAmount,
      referenceDate,
      ReversedLogsRaw.internalId,
      IF(referenceDate < ConfirmationDates.settlementDate, "reversedBeforeSettlement", "reversedAfterSettlement") AS reverstionSituation,
    FROM
      ReversedLogsRaw
  ...
  Confirmed AS (
    ...
    FROM
      PurchasesCollection
      LEFT JOIN PurchasesLog ON PurchasesCollection.endToEndId = PurchasesLog.endToEndId AND PurchasesCollection.logCreated = PurchasesLog.logCreated AND type = "confirmed"
    GROUP BY ALL
  ),
